...
public final class Parser {
    ...
    /**
     ...
     It will take care of memory usage, although the rules about not using `HLSTag`
    ...
    public func update(eventVariantPlaylist: VariantPlaylist,
        ...
        guard let result = playlist else {
            ...
            throw ParserError.unknown(description: "No error found, but no playlist was generated.")
        }
     ...
     * R: A "result" type that you define for yourself. It's typically a enum
     ...
     `HLSTag` array OR a parser error, your custom data class, a RegisteredHLSTags
...
/**
 ...
 a RegisteredHLSTags, and the original Data used to produce the `HLSTag` array.
 ...
 The `BaseParserResult` is going to either have an array of `HLSTag`s or a `ParserError`.
...
public typealias PlaylistParserFailure = (ParserError) -> (Swift.Void)


fileprivate final class ParseWorker: NSObject, HLSRapidParserCallback {
    ...
    func addedNoValueTag(withName tagName: HLSStringRef) {
        ...
        guard descriptor.type() == .noValue else {
            parseError = ParserError.mismatchBetweenTagDescriptorAndTagData(description:"The HLS Tag and the data contained within do not match: tagName:\"\(tagName.stringValue())\" tagValue:<no tag value> descriptor:\(descriptor)")
            return
        }
        tags.append(HLSTag(tagDescriptor: descriptor, tagData: HLSStringRef(), tagName: scrubHLSStringRef(tagName)))
    }
    ...
    func addedTag(withName tagName: HLSStringRef, value: HLSStringRef) {
        ...
        let descriptor = tagDescriptor(forTagName: tagName)
        guard descriptor.type() != .noValue else {
            parseError = ParserError.mismatchBetweenTagDescriptorAndTagData(description:"The HLS Tag and the data contained within do not match: tagName:\"\(tagName.stringValue())\" tagValue:\"\(value.stringValue())\" descriptor:\(descriptor)")
            return
        }
    ...
    func parseError(_ error: String, errorNumber: UInt32) {
        ...
        case ParserInternalErrorCode.missingTagData.rawValue:
            parsererror = .missingTagData(description:error)
            break
        case ParserInternalErrorCode.missingTagDataForEXTINF.rawValue:
    ...
    private func parseFail(error: ParserError) {
        failure(error)
    ...
    private func parseTags(tagValue: HLSStringRef, descriptor: HLSTagDescriptor) -> HLSTagDictionary? {
        ...
        do {
            ...
            parseError = ParserError.unknown(description:"Unknown error: \"\(error)\" while parsing tag \"\(descriptor.toString())\" with body \"\(String(describing: tagValue))\"")
        }
        return nil
    }
    
    private var parseError: ParserError? = nil
