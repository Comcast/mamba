...
class HLSPlaylistAggregateTagCardinalityValidator: HLSPlaylistValidator {
    
    static let validations: [HLSPlaylistTagCardinalityValidation.Type] = [EXT_X_MEDIA_SEQUENCEValidation.self,
    ...
    public static func validate(hlsPlaylist: HLSPlaylistInterface) -> [HLSValidationIssue]? {
        ...
        let validator = HLSPlaylistCardinalityValidator()
        let tags = hlsPlaylist.tags.filter { (tag) -> Bool in validations.contains(where: { $0.tagDescriptor == tag.tagDescriptor }) }
...
class HLSPlaylistCardinalityValidator {
    
    func validate(validation: HLSPlaylistTagCardinalityValidation.Type, tags: [HLSTag]) -> HLSValidationIssue? {
...
class EXT_X_MEDIA_SEQUENCEValidation: HLSPlaylistTagCardinalityValidation {
...
class EXT_X_DISCONTINUITY_SEQUENCEValidation: HLSPlaylistTagCardinalityValidation {
...
class EXT_X_ENDLISTValidation: HLSPlaylistTagCardinalityValidation {
...
class EXT_X_VERSIONValidation: HLSPlaylistTagCardinalityValidation {
...
class EXT_X_TARGETDURATIONValidation: HLSPlaylistTagCardinalityValidation {
