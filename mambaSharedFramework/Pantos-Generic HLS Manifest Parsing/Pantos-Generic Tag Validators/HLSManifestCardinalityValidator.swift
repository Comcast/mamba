...
class HLSManifestAggregateTagCardinalityValidator: HLSManifestValidator {
    
    static let validations: [HLSManifestTagCardinalityValidation.Type] = [EXT_X_MEDIA_SEQUENCEValidation.self,
    ...
    public static func validate(hlsManifest: HLSManifestInterface) -> [HLSValidationIssue]? {
        ...
        let validator = HLSManifestCardinalityValidator()
        let tags = hlsManifest.tags.filter { (tag) -> Bool in validations.contains(where: { $0.tagDescriptor == tag.tagDescriptor }) }
...
class HLSManifestCardinalityValidator {
    
    func validate(validation: HLSManifestTagCardinalityValidation.Type, tags: [HLSTag]) -> HLSValidationIssue? {
...
class EXT_X_MEDIA_SEQUENCEValidation: HLSManifestTagCardinalityValidation {
...
class EXT_X_DISCONTINUITY_SEQUENCEValidation: HLSManifestTagCardinalityValidation {
...
class EXT_X_ENDLISTValidation: HLSManifestTagCardinalityValidation {
...
class EXT_X_VERSIONValidation: HLSManifestTagCardinalityValidation {
...
class EXT_X_TARGETDURATIONValidation: HLSManifestTagCardinalityValidation {
