...
class  EXT_X_STARTTimeOffsetValidator: HLSManifestValidator {
    ...
    static func validate(hlsManifest manifest: HLSManifestInterface) -> [HLSValidationIssue]? {
        ...
        if let range = manifest.footer?.range {
        ...
        let latestAllowedStartTime = manifest.endTime - CMTimeMultiply(targetDuration, 3)
        
        if (startTimeOffSet > manifest.endTime) || (!endListExist && startTimeOffSet > latestAllowedStartTime) {
