...
class EXT_X_TARGETDURATIONLengthValidator: VariantPlaylistOneToManyValidator {
    ...
    static let oneTagDescriptor: PlaylistTagDescriptor = PantosTag.EXT_X_TARGETDURATION
    static let manyTagDescriptor: PlaylistTagDescriptor = PantosTag.EXTINF
    
    class var validation: (PlaylistTag?, [PlaylistTag]?) -> [PlaylistValidationIssue] {
        
        return { (one, many) -> [PlaylistValidationIssue] in
            ...
            for tag in many {
                if tag.duration.seconds.rounded(.toNearestOrAwayFromZero) > max {
                    return [PlaylistValidationIssue(description: IssueDescription.EXT_X_TARGETDURATIONLengthValidator, severity: IssueSeverity.error)]
                }
            }
