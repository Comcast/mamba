...
public class GenericSingleTagValidator<S: FailableStringLiteralConvertible>: PlaylistTagValidator {
    
    private let singleValueIdentifier: PlaylistTagValueIdentifier
    ...
    public func validate(tag: PlaylistTag) -> [PlaylistValidationIssue]? {
        ...
        if tag.numberOfParsedValues() < 1 {
            issueList.append(PlaylistValidationIssue(description: "\(tag.tagDescriptor.toString()) has no parsed data.", severity: IssueSeverity.error))
        }
        else if tag.numberOfParsedValues() > 1 {
            issueList.append(PlaylistValidationIssue(description: "\(tag.tagDescriptor.toString()) has more than one parsed data values.", severity: IssueSeverity.warning))
        }
