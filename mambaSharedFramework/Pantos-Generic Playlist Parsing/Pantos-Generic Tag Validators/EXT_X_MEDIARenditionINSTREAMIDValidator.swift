...
class EXT_X_MEDIARenditionINSTREAMIDValidator: MasterPlaylistTagGroupValidator {
    ...
    class var validation: ([PlaylistTag]) -> [PlaylistValidationIssue] {
        return { (tags) -> [PlaylistValidationIssue] in
            ...
            let variantPlaylistTag: [PlaylistTag]  = tags.filter { (tag) -> Bool in tag.value(forValueIdentifier: PantosValue.type) == MediaType.Media.ClosedCaptions.rawValue  }
            for tag in variantPlaylistTag {
                guard let _ :String = tag.value(forValueIdentifier: PantosValue.instreamId) else {
                    return [PlaylistValidationIssue(description: IssueDescription.EXT_X_MEDIA_InstreamIdValidation, severity: IssueSeverity.error)]
                }
            }
