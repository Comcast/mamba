...
public protocol TagGroupProtocol {
    /**
     The range of this group within the HLSManifestInterface's `tags` array.
...
/**
 An object to model a group of related and contiguous tags within a HLSManifestInterface.
 */
...
/**
 An object to model a group of related and contiguous tags that represent a "media fragment"
 within a HLSManifestInterface.
 
 Note that we currently also use this to model variant manifest "groups" in master manifests.
 This is convenient but not semantically correct.
 */
public struct MediaFragmentTagGroup: TagGroupProtocol, CustomDebugStringConvertible {
    ...
    public var debugDescription: String {
        return "MediaFragmentTagGroup startIndex: \(startIndex) endIndex:\(endIndex) mediaSequence:\(mediaSequence) timeRange:\(timeRange) discontinuity:\(discontinuity)"
    }
}
