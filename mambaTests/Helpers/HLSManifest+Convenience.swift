...
public extension HLSManifestCore where T == HLSManifestURLData {
    ...
    init() {
        ...
        self.init(url: fakeManifestURL(), tags: [HLSTag](), registeredTags: registeredTags, hlsData: Data())
    }
    
    init(tags: [HLSTag]) {
        let registeredTags = RegisteredHLSTags()
        self.init(url: fakeManifestURL(), tags: tags, registeredTags: registeredTags, hlsData: Data())
    }
